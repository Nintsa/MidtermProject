    ...
    private fun readSession() {
        viewModelScope.launch {
            getTokenUseCase().collect {
                if (it.isEmpty())
                    _uiEvent.emit(SplashUiEvent.NavigateToHomePage)
                else
                    _uiEvent.emit(SplashUiEvent.NavigateToChooseQuiz)
            }
        }
    }

    sealed interface SplashUiEvent {
        object NavigateToHomePage : SplashUiEvent
        object NavigateToChooseQuiz : SplashUiEvent
